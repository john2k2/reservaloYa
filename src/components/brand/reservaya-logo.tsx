  ...
  return (
    <svg viewBox="0 0 64 64" fill="none" aria-hidden="true" {...props}>
      <path
        ...
        strokeLinecap="round"
      />
      <rect x="14" y="13" width="39" height="39" rx="11" stroke="var(--sello)" strokeWidth="5" />
      <path d="M24 9.5V19M43 9.5V19" stroke="var(--sello)" strokeWidth="5" strokeLinecap="round" />
      <path
...
export function ReservaYaLogo({ variant = "full", size = "md", className }: ReservaYaLogoProps) {
  if (variant === "isotype") {
    return (
      <ReservaYaIsotypeSvg
        className={cn(sizeClasses[size].isotype, "shrink-0 text-foreground", className)}
        role="img"
  ...
  return (
    <span className={cn("inline-flex items-center gap-2 text-foreground", className)} aria-label="ReservaYa">
      <ReservaYaIsotypeSvg className={cn(sizeClasses[size].isotype, "shrink-0")} />
      <ReservaYaWordmarkSvg className={cn(sizeClasses[size].full, "hidden sm:block")} />
